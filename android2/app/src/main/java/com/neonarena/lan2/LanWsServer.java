...
public class LanWsServer extends WebSocketServer {
    static class P { WebSocket ws; String name="Игрок"; double x,y,hp=100,ix,iy; int score; long lastShot; }
    ...
    private String cleanName(String s){if(s==null||s.trim().isEmpty())return "Игрок";s=s.trim();return s.length()>18?s.substring(0,18):s;}
    @Override public void onMessage(WebSocket c,String text){try{
        JSONObject m=new JSONObject(text); String t=m.optString("type"); String name=cleanName(m.optString("name","Игрок"));
        if("create".equals(t)){if(p[0].ws==null){p[0].ws=c;p[0].name=name;p[0].x=250;p[0].y=350;send(c,"{\"type\":\"waiting\"}");}return;}
        if("join".equals(t)){if(p[0].ws!=null&&p[1].ws==null){p[1].ws=c;p[1].name=name;p[1].x=750;p[1].y=350;p[0].hp=p[1].hp=100;running=true;send(p[0].ws,"{\"type\":\"started\",\"player\":0}");send(p[1].ws,"{\"type\":\"started\",\"player\":1}");}else send(c,"{\"type\":\"error\",\"message\":\"Игра уже занята или ещё не создана.\"}");return;}
    ...
    void tick(){if(!running||p[0].ws==null||p[1].ws==null)return;double dt=.05;for(P a:p){double l=Math.hypot(a.ix,a.iy);if(l>0){a.x=Math.max(25,Math.min(975,a.x+a.ix/l*300*dt));a.y=Math.max(70,Math.min(675,a.y+a.iy/l*300*dt));}}Iterator<B> it=shots.iterator();while(it.hasNext()){B b=it.next();b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;P target=p[1-b.owner];if(Math.hypot(b.x-target.x,b.y-target.y)<28){it.remove();target.hp-=20;if(target.hp<=0){p[b.owner].score++;p[0].hp=p[1].hp=100;p[0].x=250;p[1].x=750;shots.clear();}}else if(b.life<=0||b.x<-50||b.x>1050||b.y<20||b.y>750)it.remove();}try{JSONObject d=new JSONObject().put("type","snap");JSONArray ps=new JSONArray();for(P q:p)ps.put(new JSONObject().put("x",q.x).put("y",q.y).put("hp",q.hp).put("score",q.score).put("name",q.name));d.put("players",ps);JSONArray bs=new JSONArray();for(B b:shots)bs.put(new JSONObject().put("x",b.x).put("y",b.y).put("owner",b.owner));d.put("bullets",bs);for(P q:p)send(q.ws,d.toString());}catch(Exception ignored){}}
}
