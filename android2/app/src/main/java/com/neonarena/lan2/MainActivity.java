...
public class MainActivity extends Activity {
    ...
    private void startServers(){
        if(started)return;
        try{
            http=new LanHttpServer(this,HTTP_PORT); http.start();
            ws=new LanWsServer(WS_PORT); ws.start(); started=true;
            web.evaluateJavascript("window.serverReady && window.serverReady("+JSONObjectQuote(getLanUrl())+")",null);
        }catch(Exception e){ Toast.makeText(this,"Не удалось запустить сервер: "+e.getMessage(),Toast.LENGTH_LONG).show(); }
    }
    private String getLanIp(){
        try{
            Enumeration<NetworkInterface> ns=NetworkInterface.getNetworkInterfaces(); String fallback=null;
            while(ns.hasMoreElements()){
                NetworkInterface n=ns.nextElement(); String name=n.getName().toLowerCase(); Enumeration<InetAddress> as=n.getInetAddresses();
                while(as.hasMoreElements()){
                    InetAddress a=as.nextElement(); if(!(a instanceof Inet4Address)||a.isLoopbackAddress())continue; String ip=a.getHostAddress();
                    if(ip.startsWith("192.168.")||ip.startsWith("10.")||ip.matches("172\\.(1[6-9]|2[0-9]|3[0-1])\\..*")){
                        if(name.contains("wlan")||name.contains("wifi")||name.contains("ap")||name.contains("swlan"))return ip; if(fallback==null)fallback=ip;
                    }
                }
    ...
    private String JSONObjectQuote(String x){return "\\\""+x.replace("\\","\\\\").replace("\"","\\\"")+"\\\"";}
