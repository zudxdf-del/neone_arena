...
func _ready():
    _arena(); _camera(); _menu(); _hud(); _network(); menu.visible=true; hud.visible=false

func _process(delta):
    _animate()
    if in_match:
        _controls(); _render_players(); _render_bullets(); _camera_follow(delta); _hud_update()

func _create_room():
    if not connected: status_label.text="СЕРВЕР НЕ ПОДКЛЮЧЕН"; return
    var nick=nick_edit.text.strip_edges(); if nick=="": nick="Игрок"
    var rn=room_name_edit.text.strip_edges(); if rn=="": rn="Неоновая арена"
...
func _network():
    net=NET_SCRIPT.new(); add_child(net); net.message_received.connect(_message)
    ...
    net.failed.connect(func(r): status_label.text=str(r)); net.connect_to_server()
...
func _arena():
    ...
    _border(); for o in OBS: _obstacle(o)
...
func _hud():
    hud=Control.new(); hud.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); add_child(hud); var top:=ColorRect.new(); top.size=Vector2(1280,82); top.color=Color(0.01,0.03,0.08,.94); hud.add_child(top)
    online_label=_label("",Vector2(25,18),18); hp_bar=ProgressBar.new(); hp_bar.position=Vector2(300,20); hp_bar.size=Vector2(270,30); hp_bar.max_value=100; hud.add_child(hp_bar); ammo_label=_label("",Vector2(610,15),22); weapon_label=_label("",Vector2(610,46),14); score_label=_label("",Vector2(900,18),20); event_label=_label("",Vector2(380,100),22)
    crosshair:=_label("+",Vector2(625,340),38); crosshair.modulate=Color.WHITE
    joystick_base=Panel.new(); joystick_base.position=Vector2(45,500); joystick_base.size=Vector2(190,190); joystick_base.modulate=Color(.1,.7,1,.25); hud.add_child(joystick_base); joystick_knob=Panel.new(); joystick_knob.position=Vector2(110,565); joystick_knob.size=Vector2(60,60); joystick_knob.modulate=Color(.3,1,1,.85); hud.add_child(joystick_knob)
...
func _label(t,p,size): var l:=Label.new(); l.text=t; l.position=p; l.add_theme_font_size_override("font_size",size); hud.add_child(l); return l
func _action(t,p,s): var b:=Button.new(); b.text=t; b.position=p; b.size=s; b.add_theme_font_size_override("font_size",18); hud.add_child(b); return b
func _controls():
    var mv:=joystick_vec
...
func _render_players():
    for i in range(players.size()):
        ...
        var label:Label3D=v.get_node("Name"); label.text=("YOU  " if i==local_id else "P%d  "%[i+1])+str(p.get("name","Игрок"))+"  %dHP"%int(p.get("hp",0))
...
func _player(i):
    ...
    var visor:=_box(Vector3(.46,.14,.1),Color.WHITE,c,4); visor.position=Vector3(0,1.62,-.28); root.add_child(visor); var ring:=_ring(c,.58,.035); ring.position.y=.05; root.add_child(ring)
    ...
    var label:=Label3D.new(); label.name="Name"; label.position=Vector3(0,2.4,0); label.font_size=24; label.outline_size=7; label.billboard=BaseMaterial3D.BILLBOARD_ENABLED; label.modulate=c; root.add_child(label); return root
func _mat(c): var m:=StandardMaterial3D.new(); m.albedo_color=c; m.metallic=.3; m.roughness=.25; m.emission_enabled=true; m.emission=c; m.emission_energy_multiplier=1.8; return m
func _render_bullets():
    var alive={}; for b in bullets: alive[int(b.get("id",0))]=true
...
func _camera_follow(delta):
    ...
    if count==0:return
    var center:=Vector3((minx+maxx)*S/2,0,(miny+maxy)*S/2); var span:=max((maxx-minx)*S,(maxy-miny)*S)+7; var target_y=clamp(11+span*.55,11,24); var target=center+Vector3(0,target_y,span*.28); camera.position=camera.position.lerp(target,1-exp(-delta*3.5)); camera.look_at(center,Vector3.UP)
func _hud_update():
...
func _message(d):
    match str(d.get("type","")):
        ...
        "start": local_id=int(d.get("player",local_id)); players=d.get("players",[]); in_match=true; use_server_state=true; menu.visible=false; hud.visible=true; event_label.text="БОЙ НАЧАЛСЯ — %d ИГРОКОВ"%int(d.get("playerCount",2))
